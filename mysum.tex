...
% Surformer
This study proposes an innovative MIL neural network termed pattern-perceptive survival transformer (Surformer) \cite{atabansi2023survey}for WSI-based survival analysis. Briefly, Surformer comprises three components: a ratio-reserved cross-attention module (RRCA), a multi-head selfattention module (MHSA) , and a multi-head cross-attention module (MHCA), RRCA is the key module in Surformer. In the RRCA process, both 𝑝𝑔𝑙𝑜𝑏𝑎𝑙 and 𝑝𝑙𝑜𝑐𝑎𝑙𝑠 function as detectors to gather correlative features from WSIs.Here, the 𝑝𝑔𝑙𝑜𝑏𝑎𝑙 is similar to the classification token of Bert\cite{devlin2018bert} and targets to aggregate all valuable features for survival prediction.RRCA simultaneously detects global features and multiple pattern-specific local features through a learnable global prototype 𝑝𝑔𝑙𝑜𝑏𝑎𝑙 and multiple local prototypes 𝑝𝑙𝑜𝑐𝑎𝑙𝑠 and quantifies the patches correlative to each 𝑝𝑙𝑜𝑐𝑎𝑙𝑠 in the form of ratio factors (rfs). The ratio information is subsequentially embedded into the feature space for representation enhancement. 
